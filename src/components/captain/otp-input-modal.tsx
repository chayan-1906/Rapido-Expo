...
function OtpInputModal({title, visible, onConfirm, onClose}: OtpInputModalProps) {
                ...
                <View style={styles.otpContainer}>
                    {otp.map((digit, index) => (
                        <TextInput key={index} ref={(ref) => (inputs.current[index] = ref)}
                                   value={digit} onChangeText={(value) => handleOtpChange(value, index)}
                                   style={styles.otpInput} keyboardType={'numeric'} maxLength={1}
                        />
                    ))}
                </View>
